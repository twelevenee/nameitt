...
import { ArrowLeft, TrendingUp, Brain, MapPin, ChevronDown, Send, BookText } from "lucide-react";
...
const CHART_COLORS = [
  "hsl(230, 30%, 65%)", "hsl(230, 25%, 72%)", "hsl(230, 20%, 78%)",
  "hsl(230, 30%, 58%)", "hsl(230, 25%, 68%)", "hsl(230, 20%, 75%)", "hsl(230, 15%, 80%)",
];
...
const EducationalCard = ({
  patternKey, title, explanation, examples, reportedCount, totalExperiences, showStats,
}: {
  patternKey: string; title: string; explanation: string; examples: string[];
  reportedCount: number; totalExperiences: number; showStats: boolean;
}) => {
            ...
            {examples.length > 0 && (
              <div className="space-y-2">
                ...
                <ul className="space-y-1.5">
                  {examples.map((ex, i) => (
                    <li key={i} className="text-xs sm:text-sm text-foreground/70 leading-relaxed pl-3 border-l-2 border-primary/20 break-words">"{ex}"</li>
                  ))}
                </ul>
              </div>
            )}
            {showStats && reportedCount > 0 && (
              <div className="space-y-2 pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-xs text-muted-foreground">{reportedCount} {reportedCount === 1 ? "report" : "reports"}</p>
                </div>
      ...
      <div key={item.name} className="space-y-1">
        ...
        <div className="h-2 rounded-full bg-secondary overflow-hidden">
          <div className="h-full rounded-full transition-all duration-500"
            style={{ width: `${maxCount > 0 ? (item.count / maxCount) * 100 : 0}%`, backgroundColor: CHART_COLORS[i % CHART_COLORS.length] }} />
        </div>
      </div>
    ))}
  </div>
...
const Patterns = () => {
  ...
  useEffect(() => { document.title = "Anonymous Patterns — Was I Too Sensitive?"; }, []);

  useEffect(() => {
    const fetchStats = async () => {
      try {
        const { data: experiences } = await supabase
          .from("experiences").select("id, context_where, self_doubt").eq("contributed", true);
        if (!experiences || experiences.length === 0) { setStats(null); setLoading(false); return; }

        const expIds = experiences.map((e) => e.id);
        const { data: analyses } = await supabase
          .from("analyses").select("detected_patterns, self_doubt_detected").in("experience_id", expIds);
        ...
        for (const a of analyses ?? []) {
          ...
          if (Array.isArray(patterns)) { for (const p of patterns) { patternMap[p] = (patternMap[p] || 0) + 1; } }
        }
        ...
        for (const e of experiences) { if (e.context_where) { contextMap[e.context_where] = (contextMap[e.context_where] || 0) + 1; } }
        ...
        setStats({
          ...
          topContext,
          patternCounts: sorted.map(([key, count]) => ({ name: PATTERNS.find((p) => p.key === key)?.title ?? key, count })),
    ...
    <div id="main-content" className="min-h-screen px-6 py-10 relative" style={{ background: "var(--gradient-warm)" }}>
      ...
      <div className="max-w-3xl mx-auto space-y-12 relative z-10">
        ...
        <div className="space-y-3">
          ...
          <p className="text-muted-foreground leading-relaxed">Based on anonymous, voluntarily shared experiences. No personal details are displayed.</p>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                {PATTERNS.map((pattern) => (
                  <EducationalCard key={pattern.key} patternKey={pattern.key} title={pattern.title}
                    explanation={pattern.explanation} examples={pattern.examples}
                    reportedCount={stats?.patternCountsByKey?.[pattern.key] ?? 0}
                    totalExperiences={stats?.totalExperiences ?? 0} showStats={showPerPatternStats} />
                ))}
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              {!hasData ? (
                <div className="rounded-2xl bg-card p-8 text-center space-y-6 shadow-[var(--shadow-soft)]">
                  <div className="flex justify-center"><WarmBlobs className="w-48 h-36 opacity-60" /></div>
                  <p className="text-sm text-muted-foreground leading-relaxed max-w-md mx-auto">As more people reflect, patterns will emerge here. Every voice matters.</p>
                  <Button asChild className="rounded-full">
                    <Link to="/reflect" className="inline-flex items-center gap-2">
                      <Send className="w-4 h-4" aria-hidden="true" />Add yours
                    </Link>
                  </Button>
                </div>
              ) : (
                <>
                  ...
                  {showChart && stats!.patternCounts.length > 0 && (
                    <div className="rounded-2xl bg-card p-5 sm:p-8 space-y-4 shadow-[var(--shadow-soft)]">
                      <h3 className="text-xs sm:text-sm font-medium text-muted-foreground">Most frequently identified patterns across all shared experiences</h3>
                      {isMobile ? (
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                          <BarChart data={stats!.patternCounts} layout="vertical" margin={{ left: 20 }}>
                            ...
                            <Tooltip contentStyle={{ background: "hsl(35, 20%, 97%)", border: "1px solid hsl(230, 15%, 90%)", borderRadius: "12px", fontSize: "13px" }} />
        ...
        <div className="rounded-2xl bg-card/60 p-5 text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">Read anonymous stories from people who reflected on similar experiences.</p>
        ...
        <footer className="space-y-3 pb-6">
          ...
          <p className="text-xs text-muted-foreground/60 text-center">This dashboard shows aggregate data only. No individual experiences are identifiable.</p>
        </footer>
      </div>
    </div>
  );
